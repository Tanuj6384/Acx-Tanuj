pageextension 60024 TransferOrdersListExt extends "Transfer Orders"
{
    trigger OnOpenPage()
    var
        GenJourTemplateSubType: Enum "Gen. Jour. Template SubType";
    begin
        GenJnlManagement.TemplateSelectionForTransfer(5740, GenJourTemplateSubType::"Transfer Order", Rec, JnlSelected, GenJnlTemplateCode);
        if not JnlSelected then
            Error('Journal template not selected.');

        recInventorySetup.Get();
        recInventorySetup."Transfer Template Temp" := GenJnlTemplateCode;
